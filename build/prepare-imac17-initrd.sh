...
(
    ...
    find usr scripts limad-installer -print0 \
        | LC_ALL=C sort -z \
        | cpio --null --quiet -o -H newc > "$WORK/limad-v17-early.cpio"
)

cat "$WORK/limad-v17-early.cpio" "$ORIGINAL_INITRD" > "$OUTPUT_INITRD"
chmod 0644 "$OUTPUT_INITRD"

if [ "$(stat -c %s "$OUTPUT_INITRD")" -le "$(stat -c %s "$ORIGINAL_INITRD")" ]; then
    echo "ERROR: LiMaD V17 initrd prefix was not added" >&2
    exit 1
fi
...
echo "V17 INITRD FIRMWARE + LIVE BRANDING: PASS"
