...
(
    ...
    find usr scripts limad-installer -print0 \
        | LC_ALL=C sort -z \
        | cpio --null --quiet -o -H newc > "$WORK/limad-v14-early.cpio"
)

cat "$WORK/limad-v14-early.cpio" "$ORIGINAL_INITRD" > "$OUTPUT_INITRD"
chmod 0644 "$OUTPUT_INITRD"

if [ "$(stat -c %s "$OUTPUT_INITRD")" -le "$(stat -c %s "$ORIGINAL_INITRD")" ]; then
    echo "ERROR: LiMaD V14 initrd prefix was not added" >&2
    exit 1
fi
...
echo "V14 INITRD FIRMWARE + LIVE BRANDING: PASS"
