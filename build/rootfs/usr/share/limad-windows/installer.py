...
WINETRICKS_DIRECTX_MIRROR_URL = "https://files.holarse-linuxgaming.de/mirrors/microsoft/directx_Jun2010_redist.exe"
WINETRICKS_DIRECTX_OFFICIAL_URL = "https://download.microsoft.com/download/8/4/A/84A35BF1-DAFE-4AE8-82AF-AD2AE20B6B14/directx_Jun2010_redist.exe"
...
def _official_cli_runners(log_file: Path) -> list[str]:
    ...
    for runner in candidates:
        if runner.startswith("sys-") or _runner_executable_host(runner):
            valid.append(runner)
    return valid


def _runner_works(runner: str, log_file: Path) -> bool:
    if runner.startswith("sys-"):
        code, output = run_host_process(
            ["flatpak", "run", "--command=sh", BOTTLES_APP_ID, "-lc", "wine --version"],
            log_file,
            60,
        )
        works = code == 0 and "wine" in output.lower()
        log(f"runner probe {runner}/wine: {'ok' if works else 'failed'}", log_file)
        return works
    ...
    for executable in executables:
        code, output = run_host_process(
            [
                "flatpak", "run", "--command=sh", BOTTLES_APP_ID,
                "-lc", '"$1" --version', "sh", str(executable),
            ],
            log_file,
            60,
        )
        works = code == 0 and "wine" in output.lower()
        log(f"runner probe {runner}/{executable.name}: {'ok' if works else 'failed'}", log_file)
        if not works:
            return False
    return True


def _usable_runners(runners: list[str], log_file: Path) -> list[str]:
    return [runner for runner in runners if _runner_works(runner, log_file)]


def _preferred_runner(runners: list[str], prefer_system: bool = False) -> str:
    def score(name: str) -> tuple[int, str]:
        lower = name.lower()
        if prefer_system and lower.startswith("sys-wine"):
            return (0, lower)
...
def bootstrap_bottles_components(
    ...
    prefer_system_runner: bool = False,
) -> tuple[bool, str, str]:
    runners = _usable_runners(
        list(dict.fromkeys(_official_cli_runners(log_file) + _scan_managed_runners())),
        log_file,
    )
    if runners:
        runner = _preferred_runner(runners, prefer_system_runner)
        if progress:
    ...
    while time.monotonic() < deadline:
        ...
        runners = _usable_runners(runners, log_file)
        if runners:
            runner = _preferred_runner(runners, prefer_system_runner)
...
def _patch_winetricks_directx_source(target: Path, log_file: Path) -> tuple[bool, str]:
    try:
        content = target.read_text(encoding="utf-8")
    except OSError as exc:
        return False, f"Winetricks konnte nicht gelesen werden: {exc}"
    mirror_count = content.count(WINETRICKS_DIRECTX_MIRROR_URL)
    official_count = content.count(WINETRICKS_DIRECTX_OFFICIAL_URL)
    if mirror_count == 0 and official_count >= 1:
        return True, ""
    if mirror_count != 1:
        return False, (
            "Die erwartete DirectX-Downloadquelle wurde in Winetricks nicht eindeutig gefunden. "
            f"Treffer: {mirror_count}."
        )
    patched = content.replace(
        WINETRICKS_DIRECTX_MIRROR_URL,
        WINETRICKS_DIRECTX_OFFICIAL_URL,
        1,
    )
    ...
    try:
        ...
        return False, f"Die DirectX-Downloadquelle in Winetricks konnte nicht korrigiert werden: {exc}"
    log("winetricks directx source: official Microsoft download", log_file)
    return True, ""
...
def _winetricks_script(log_file: Path, progress: Callable[[str, float], None] | None = None) -> tuple[bool, str, Path | None]:
    ...
    if target.is_file():
        try:
            ...
            if f"WINETRICKS_VERSION={WINETRICKS_VERSION}" in head:
                ok, error = _patch_winetricks_directx_source(target, log_file)
                if not ok:
    ...
    try:
        request = urllib.request.Request(WINETRICKS_ARCHIVE_URL, headers={"User-Agent": "LiMaD-Windows-Programme/2.2.8"})
...
def _run_winetricks_dependency(
    ...
    if runner.startswith("sys-"):
        ...
        sandbox_wine = str(sandbox_root / relative_runner)
        sandbox_wineserver = str((sandbox_root / relative_runner).with_name("wineserver"))
...
def ensure_bottle_cli(
    ...
    log_file: Path,
    progress: Callable[[str, float], None] | None = None,
    environment_type: str = "application",
) -> tuple[bool, str]:
    ...
    args = [
        "flatpak", "run", "--command=bottles-cli", BOTTLES_APP_ID,
        "new", "--bottle-name", bottle, "--environment", environment_type,
...
class InstallerWindow(Adw.ApplicationWindow):
    ...
    def task_install_bottles(self) -> str:
        ...
        ok, error, runner = bootstrap_bottles_components(
            log_file,
            self.set_progress,
            prefer_system_runner=plan.recipe == "nws",
        )
        if not ok:
        ...
        ok, error = ensure_bottle_cli(
            bottle,
            runner,
            log_file,
            self.set_progress,
            environment_type="custom" if plan.recipe == "nws" else "application",
        )
        if not ok:
